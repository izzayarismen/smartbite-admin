...
function VerifikasiPage() {
    ...
    try {
      ...
      if (res.ok && result.success) {
        const formattedData = result.data.map((toko: any) => ({
          ...
          storePhoto:
            toko.foto_url || "https://images.unsplash.com/photo-1578916171728-46686eac8d58?w=500",
          ktpPhoto:
            toko.foto_ktp_url || "https://images.unsplash.com/photo-1554774853-aae0a22c8aa4?w=500",
          selfiePhoto:
            toko.foto_diri_url ||
  ...
  const historyCols: Column<VerifyHistory>[] = [
    ...
    },
    {
      key: "date",
      header: "Tanggal",
      render: (r) => <span className="text-muted-foreground">{r.verifiedAt}</span>,
    },
  ];
      ...
      <div className="mb-6 inline-flex rounded-2xl border border-border bg-card/90 p-1 backdrop-blur-md">
        {tabs.map((t) => (
          ...
          >
            ...
            <span
              className={`relative z-10 ${tab === t.key ? "text-primary-foreground" : "text-muted-foreground"}`}
            >
              {t.label}
                ...
                <div onClick={() => setSelected(s)}>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    <div className="overflow-hidden rounded-xl border border-border">
                      ...
                      <p className="bg-accent/40 py-1 text-center text-[10px] font-medium text-muted-foreground">
                        Foto KTP
                      </p>
                    </div>
                    <div className="overflow-hidden rounded-xl border border-border">
                ...
                {!rejecting ? (
                  <>
                    <button
                      ...
                      className="flex flex-1 items-center justify-center gap-2 rounded-2xl gradient-brand py-3 text-sm font-bold text-primary-foreground shadow-glow transition-transform hover:scale-[1.02]"
                    >
                      <Check className="h-4 w-4" /> Approve
                    </button>
                    <button
                      onClick={() => setRejecting(true)}
                      className="flex flex-1 items-center justify-center gap-2 rounded-2xl border border-destructive/30 bg-destructive/10 py-3 text-sm font-bold text-destructive transition-colors hover:bg-destructive/20"
                    >
